...
header("Location: ../");
  ...
  if (isset($_FILES["profile_picture"]) && parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST) == $domain) {
   $resault_display = "block";
    $valid_file_types = array(
     ...
     $size = is_array(getimagesize($_FILES["profile_picture"]["tmp_name"]));
    if (in_array($_FILES['profile_picture']['type'],$valid_file_types) && $size == true ) {
       $user_picture =  $user_name . "_"  .  rand(1,99999) . '.' . end(explode(".",$_FILES["profile_picture"]["name"]));
       move_uploaded_file($_FILES['profile_picture']['tmp_name'], '../profile_pictures_for_users/' . $user_picture);
       ...
       // uddate picture database
       $query_update_pic = "UPDATE tablefor_users set thisisuser_picture='".$user_picture."' where id=" . $id["id"];
       $execute_update = mysqli_query($db, $query_update_pic);
       if ($execute_update == true) {
         $icon = "true-icon.png";
         $resault = $pic_update_success;
       }else {
          $icon = "error-icon.png";
         $resault = $failed_register;
       }
...
<li class="border-left my-messages" style="position:relative;width:100%;border-left:0"><h4><a class="back" href="../settings" ><?php echo $back; ?></a><?php echo $change_pic_label; ?></h4></li>
...
<center>
<div class="error-report" style="margin-bottom:0px;display:<?php echo $resault_display; ?>"><span><?php echo $resault; ?></span><img class="error-icon" src="../images/<?php echo $icon; ?>" width="20" height="20" /></div>
</center>
...
<span><?php echo $select_pic; ?></span>
<form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
<input type="file" name="profile_picture" accept="image/*" style="width:100%;padding:12px;background-color:#ed2553;color:white" />

<button style="background-color:white;color:#ed2553;width:100%"><?php echo $update_pic_label; ?></button>
